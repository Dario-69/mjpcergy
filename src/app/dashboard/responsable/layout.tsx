  ...
  return (
    <ProtectedRoute requiredRole="responsable">
      <div className="min-h-screen">
        {/* Mobile sidebar avec glassmorphism */}
        <div className={`fixed inset-0 z-50 lg:hidden transition-all duration-300 ${sidebarOpen ? 'block' : 'hidden'}`}>
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm" onClick={() => setSidebarOpen(false)} />
          <div className="fixed inset-y-0 left-0 flex w-72 flex-col glass border-r border-white/20 shadow-2xl animate-fade-in-left">
            <div className="flex h-16 items-center justify-between px-6">
              ...
              <button 
                ...
                className="p-2 rounded-lg hover:bg-white/20 transition-all-smooth"
              >
              ...
              {navigation.map((item, index) => {
                ...
                return (
                  <Link
                    key={item.name}
                    href={item.href}
                    className={`flex items-center px-4 py-3 rounded-xl text-sm font-medium transition-all-smooth group ${
                      isActive
            ...
            <div className="border-t border-white/20 p-4">
              <div className="flex items-center mb-4">
                <div className="flex-shrink-0">
                  <div className="h-10 w-10 rounded-full bg-gradient-to-r from-indigo-500 to-purple-600 flex items-center justify-center shadow-medium">
        ...
        <div className="lg:pl-72">
          {/* Top bar moderne */}
          <div className="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 glass border-b border-white/20 px-4 shadow-soft sm:gap-x-6 sm:px-6 lg:px-8">
            <button
              type="button"
              className="-m-2.5 p-2.5 text-gray-700 lg:hidden hover:bg-white/20 rounded-lg transition-all-smooth"
